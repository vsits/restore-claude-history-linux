...
- **e2e harness landed (2026-06-02):** QEMU/KVM Layer 3 harness under `tests/e2e/` executes each backend's opt-in real-host test inside a real Linux kernel (Ubuntu 24.04 cloud image + cloud-init + seed ISO + writable overlay; ~10 min total for all three backends on KVM). All three v1 backends verified end-to-end against real ZFS / Btrfs / Timeshift snapshots — see `docs/plans/qemu-e2e-plan.md` for the design and `tests/e2e/README.md` for the run instructions. Not a per-PR gate; runs before tagging a release. v1.0.0 tag pending the upstream cross-reference PR.
